...
class PyqTerminal(QWidget):
    ...
    def _send_mouse_event(self, event, is_press=False, is_release=False, is_move=False):
        ...
        if is_press or is_release:
            if event.button() == Qt.LeftButton: cb = 0
            elif event.button() == Qt.MiddleButton: cb = 1
            elif event.button() == Qt.RightButton: cb = 2
        else: # is_move
            if event.buttons() & Qt.LeftButton: cb = 0
            elif event.buttons() & Qt.MiddleButton: cb = 1
            elif event.buttons() & Qt.RightButton: cb = 2
        ...
        if event.modifiers() & Qt.AltModifier: cb += 8
        if event.modifiers() & Qt.ControlModifier: cb += 16
    ...
    def _is_selected(self, row, col, sel_range):
        if not sel_range: return False
        (r1, c1), (r2, c2) = sel_range
        if r1 < row < r2: return True
        if row == r1 == r2: return c1 <= col <= c2
    ...
    def paintEvent(self, event):
        ...
        for y in range(min_y, max_y + 1):
            ...
            for x in range(self.cols):
                ...
                if char.reverse: bg_color = self._get_color(char.fg, is_bg=False)
                if self._is_selected(y, x, sel_range):
                    fg_temp = self._get_color(char.fg, is_bg=False)
                    if char.reverse: fg_temp = self._get_color(char.bg, is_bg=True)
                    bg_color = fg_temp
            ...
            for x in range(self.cols):
                ...
                if char.data != ' ' and char.data != '':
                    ...
                    if self._is_selected(y, x, sel_range):
                        ...
                        if char.reverse: bg_temp = self._get_color(char.fg, is_bg=False)
                        fg_color = bg_temp
                        
                    # Custom rendering for block elements
                    if char.data == '\u2588': painter.fillRect(rect, fg_color); continue
                    elif char.data == '\u2580': painter.fillRect(QRect(rect.left(), rect.top(), rect.width(), (rect.height() + 1) // 2), fg_color); continue
                    elif char.data == '\u2584': 
                        hh = (rect.height() + 1) // 2
                        painter.fillRect(QRect(rect.left(), rect.top() + hh, rect.width(), rect.height() - hh), fg_color); continue
                    elif char.data == '\u258c': painter.fillRect(QRect(rect.left(), rect.top(), (rect.width() + 1) // 2, rect.height()), fg_color); continue
                    elif char.data == '\u2590':
                        hw = (rect.width() + 1) // 2
                        painter.fillRect(QRect(rect.left() + hw, rect.top(), rect.width() - hw, rect.height()), fg_color); continue

                    # Dynamic Font Styling (Cached)
                    font_key = (char.bold, char.italics, char.underscore, char.strikethrough)
                    if font_key != last_font_key:
                        if font_key == (False, False, False, False):
                            ...
                            if font_key not in self._font_cache:
                                ...
                                f.setBold(char.bold); f.setItalic(char.italics); f.setUnderline(char.underscore); f.setStrikeOut(char.strikethrough)
                    ...
                    if is_wide:
                        ...
                        x_offset = rect.left() + (self.char_width * 2 - char_real_width) / 2
