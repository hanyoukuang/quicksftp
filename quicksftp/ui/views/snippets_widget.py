...
class SnippetDialog(QDialog):
    ...
    def __init__(
        ...
        self.group_combo.setPlaceholderText("留空为默认，可输入如: Docker")
        if existing_groups:
            self.group_combo.addItems(list(existing_groups))
        ...
        layout.addRow("命令名称:", self.name_edit)
        layout.addRow("具体命令:", self.cmd_edit)
...
class QuickSnippetsWidget(QWidget):
    ...
    def _get_all_groups(self):
        groups = set()
        for snip in self.data.get("global", []):
            if snip.get("group"):
                groups.add(snip["group"])
        for snip in self.data.get("sites", {}).get(self.site_id, []):
            if snip.get("group"):
                groups.add(snip["group"])
        return groups
    ...
    def show_context_menu(self, pos):
        ...
        if item:
            ...
            if meta:
                ...
                default_group = meta.get("group", "")
            else:
                if "本站" in item.text(0):
                    default_scope = "site"
                elif "全局" not in item.text(0):
                    # 可能是分组节点
                    default_group = item.text(0).replace("📁 ", "")
                    # 尝试推断所在作用域
                    parent = item.parent()
                    if parent and "本站" in parent.text(0):
                        default_scope = "site"
    ...
    def add_snippet(self, default_scope="global", default_group=""):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if name and cmd:
                ...
                target_list.append({"name": name, "cmd": cmd, "group": group})
                self.save_snippets()
                self.refresh_tree()
    ...
    def _build_tree_nodes(self, parent_item, snippets_list, scope_str):
        groups = {}
        root_items = []
        for idx, snip in enumerate(snippets_list):
    ...
    def refresh_tree(self):
        ...
        global_root = QTreeWidgetItem(self.tree, ["🌐 全局命令 (所有服务器)"])
        global_root.setFlags(Qt.ItemFlag.ItemIsEnabled)
        self._build_tree_nodes(global_root, self.data["global"], "global")

        site_root = QTreeWidgetItem(self.tree, ["💻 本站专属"])
        site_root.setFlags(Qt.ItemFlag.ItemIsEnabled)
        self._build_tree_nodes(site_root, self.data["sites"][self.site_id], "site")
