...
from PySide6.QtWidgets import QHBoxLayout, QLabel, QProgressBar, QFrame
...
class SystemMonitorWidget(QFrame):
    ...
    def _poll_data(self):
        if (
            self.info is None
            or not self.info.connect_is_ready
            or not hasattr(self.info, "loop")
        ):
            return
            ...
            if self._last_time > 0:
                ...
                if dt > 0:
                    ...
                    self.net_label.setText(
                        f"⬆️ {self._format_speed(tx_speed)}  ⬇️ {self._format_speed(rx_speed)}"
                    )
            self._last_rx = rx
            self._last_tx = tx
            self._last_time = now
